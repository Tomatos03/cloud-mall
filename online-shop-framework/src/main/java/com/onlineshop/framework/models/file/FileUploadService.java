...
public class FileUploadService implements IFileUploadService {
    ...
    public Avatar uploadImage(MultipartFile file) {
        ...
        if (file.getSize() > MAX_SIZE_5MB) {
            throw new BizException(BizErrorCode.FILE_SIZE_EXCEEDS_LIMIT);
        }
        String originalName = Objects.requireNonNull(file.getOriginalFilename());
        String ext = getExtension(originalName);
        if (!isAllowedExt(ext)) {
            throw new BizException(BizErrorCode.FILE_EXTENSION_NOT_ALLOWED);
        }
        String mime = file.getContentType();
        if (!isAllowedMime(mime)) {
            throw new BizException(BizErrorCode.FILE_MIME_TYPE_INVALID);
        }
