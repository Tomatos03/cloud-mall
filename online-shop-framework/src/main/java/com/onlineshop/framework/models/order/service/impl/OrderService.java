...
import com.onlineshop.framework.models.system.user.UserRole;
...
public class OrderService extends ServiceImpl<OrderMapper, Order> implements IOrderService {
     ...
     */
    @Override
    public IPage<OrderAggregateVO> pageQueryForUser(OrderParamsDTO queryDTO) {
        // TODO: 待处理
    ...
    public IPage<OrderVO> pageQuery(OrderParamsDTO queryDTO) {
        // TODO: 待处理
        UserRole role = UserRole.of(UserRole.ADMIN.getCode());
        // 构建查询条件（管理员可以查询所有订单）
        LambdaQueryWrapper<Order> wrapper = OrderQueryWrapper.build(role, queryDTO, null);
    ...
    @Override
    public IPage<OrderVO> pageQueryForMerchant(OrderParamsDTO queryDTO) {
    ...
    private static void validateStore(List<Store> storeList) {
        if (CollectionUtil.isEmpty(storeList)) {
            throw new BizException(BizErrorCode.MERCHANT_NO_SHOP);
        }
    }
