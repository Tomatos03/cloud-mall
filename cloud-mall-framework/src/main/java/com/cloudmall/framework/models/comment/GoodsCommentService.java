...
import com.cloudmall.framework.models.comment.dto.CommentParamsDTO;
import com.cloudmall.framework.models.comment.dto.CreateCommentDTO;
...
import com.cloudmall.framework.models.comment.vo.GoodsCommentVO;
...
public class GoodsCommentService extends ServiceImpl<GoodsCommentMapper, GoodsComment> implements IGoodsCommentService {
    ...
    @Override
    public IPage<GoodsCommentVO> pageGoodsComment(CommentParamsDTO queryDTO) {
    ...
    private void validateCommentInput(CreateCommentDTO createCommentDTO) {
        if (createCommentDTO.getRating() == null || createCommentDTO.getRating() < 1 || createCommentDTO.getRating() > 5) {
            throw new BizException(BizErrorCode.COMMENT_RATING_INVALID);
        }

        if (!StringUtils.hasText(createCommentDTO.getContent())) {
            throw new BizException(BizErrorCode.COMMENT_CONTENT_EMPTY);
        }
    }
    ...
    private void validateOrderItem(Long orderItemId) {
        ...
        if (orderItem == null) {
            throw new BizException(BizErrorCode.COMMENT_ORDER_ITEM_NOT_EXIST);
        }
    }
    ...
    private void validateCommentNotExists(Long orderItemId) {
        ...
        if (existComment != null) {
            throw new BizException(BizErrorCode.COMMENT_ALREADY_EXISTS);
        }
    }
    ...
    private void markOrderItemCommented(CreateCommentDTO createCommentDTO) {
        boolean isSuccess = orderItemService.lambdaUpdate()
                                            ...
                                            .update();
        if (!isSuccess) {
            throw new BizException(BizErrorCode.ORDER_ITEM_NOT_EXIST);
        }
    }
